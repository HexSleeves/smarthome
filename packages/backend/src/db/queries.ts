...
export const sessionQueries = {
  ...
  findByToken: db.prepare<[string], { id: string; user_id: string; expires_at: string }>(`
...
// Helper functions
export function createUser(email: string, passwordHash: string, name?: string, role: 'admin' | 'viewer' = 'viewer'): User {
  const id = uuid();
  userQueries.create.run(id, email, passwordHash, name || null, role);
  return userQueries.findById.get(id)!;
}

export function createDevice(userId: string, type: 'roborock' | 'ring', name: string, deviceId?: string, config: object = {}): Device {
  const id = uuid();
  deviceQueries.create.run(id, userId, type, name, deviceId || null, JSON.stringify(config));
  return deviceQueries.findById.get(id)!;
}

export function createEvent(deviceId: string, type: string, data: object = {}): Event {
  const id = uuid();
  eventQueries.create.run(id, deviceId, type, JSON.stringify(data));
  return { id, device_id: deviceId, type, data: JSON.stringify(data), created_at: new Date().toISOString() };
}

export function saveCredentials(userId: string, provider: 'roborock' | 'ring', encryptedCredentials: string): void {
  const id = uuid();
  credentialQueries.upsert.run(id, userId, provider, encryptedCredentials);
}

export function getCredentials(userId: string, provider: 'roborock' | 'ring'): DeviceCredential | undefined {
