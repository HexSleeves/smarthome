...
export async function roborockRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/status', async (request) => {
    ...
    const hasCredentials = !!credentialQueries.findByProvider.get(user.id, 'roborock');
  ...
  fastify.post('/auth', async (request, reply) => {
    ...
    try {
      ...
      const result = await roborockService.authenticate(user.id, body.email, body.password);
  ...
  fastify.get('/devices', async (request, reply) => {
    ...
    if (!roborockService.isConnected(user.id)) {
      // Try to reconnect
      const connected = await roborockService.connectWithStoredCredentials(user.id);
      if (!connected) {
  ...
  fastify.post('/devices/:deviceId/command', async (request, reply) => {
    ...
    if (user.role !== 'admin') {
      return reply.status(403).send({ error: 'Admin access required for commands' });
    }
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        return reply.status(400).send({ error: 'Invalid command', details: error.errors });
      }
      return reply.status(500).send({ error: 'Command failed' });
    }
  });
  ...
  fastify.post('/devices/:deviceId/fan-speed', async (request, reply) => {
    ...
    try {
      ...
      const success = await roborockService.setFanSpeed(user.id, deviceId, body.speed);
  ...
  fastify.post('/devices/:deviceId/water-level', async (request, reply) => {
    ...
    try {
      ...
      const success = await roborockService.setWaterLevel(user.id, deviceId, body.level);
  ...
  fastify.post('/devices/:deviceId/clean-rooms', async (request, reply) => {
    ...
    try {
      ...
      const success = await roborockService.cleanRooms(user.id, deviceId, body.roomIds);
