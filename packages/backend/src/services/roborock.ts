...
export interface RoborockDeviceState {
  ...
  name: string;
  model: string;
  status: 'idle' | 'cleaning' | 'returning' | 'charging' | 'paused' | 'error' | 'offline';
  battery: number;
...
class RoborockService extends EventEmitter {
  private credentials: Map<string, RoborockCredentials> = new Map();
  private deviceStates: Map<string, RoborockDeviceState> = new Map();
  ...
  async authenticate(userId: string, email: string, password: string): Promise<{ success: boolean; error?: string }> {
    try {
  ...
  async connectWithStoredCredentials(userId: string): Promise<boolean> {
    try {
      ...
      const creds = JSON.parse(decrypt(stored.credentials_encrypted, ENCRYPTION_SECRET)) as RoborockCredentials;
      this.credentials.set(userId, creds);
  ...
  private async discoverDevices(userId: string): Promise<void> {
    ...
    try {
      ...
      for (const device of devices) {
        ...
        const existing = deviceQueries.findByType.all(userId, 'roborock')
          .find(d => d.device_id === device.duid);
        
        if (!existing) {
          createDevice(userId, 'roborock', device.name || 'Roborock Vacuum', device.duid, {
            model: device.model,
            fwVersion: device.fv,
          });
        }
  ...
  private async refreshDeviceStatus(userId: string, deviceId: string): Promise<void> {
    const creds = this.credentials.get(userId);
    if (!creds) return;
    ...
    try {
      ...
      const response = await fetch(`${ROBOROCK_API_BASE}/api/v1/user/devices/${deviceId}/status`, {
        headers: {
          'Authorization': `Bearer ${creds.token}`,
          'Accept': 'application/json',
        },
      ...
      state.errorMessage = state.errorCode ? ERROR_MESSAGES[state.errorCode] || 'Unknown error' : null;

      // Emit status update
      this.emit('statusUpdate', { userId, deviceId, state });
      
      // Update DB
      const device = deviceQueries.findByType.all(userId, 'roborock')
  ...
  async sendCommand(userId: string, deviceId: string, command: string, params: any[] = []): Promise<boolean> {
    const creds = this.credentials.get(userId);
    if (!creds) return false;

    try {
      const response = await fetch(`${ROBOROCK_API_BASE}/api/v1/user/devices/${deviceId}/command`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${creds.token}`,
          'Content-Type': 'application/json',
  ...
  async setFanSpeed(userId: string, deviceId: string, speed: RoborockDeviceState['fanSpeed']): Promise<boolean> {
    ...
    return this.sendCommand(userId, deviceId, 'set_custom_mode', [speedMap[speed]]);
  }

  async setWaterLevel(userId: string, deviceId: string, level: RoborockDeviceState['waterLevel']): Promise<boolean> {
    ...
    return this.sendCommand(userId, deviceId, 'set_water_box_custom_mode', [levelMap[level]]);
  }

  async cleanRooms(userId: string, deviceId: string, roomIds: number[]): Promise<boolean> {
    return this.sendCommand(userId, deviceId, 'app_segment_clean', [{ segments: roomIds, repeat: 1 }]);
  }
  ...
  async getCleanHistory(userId: string, deviceId: string): Promise<any[]> {
    ...
    try {
      const response = await fetch(`${ROBOROCK_API_BASE}/api/v1/user/devices/${deviceId}/clean_summary`, {
        headers: {
          'Authorization': `Bearer ${creds.token}`,
          'Accept': 'application/json',
        },
