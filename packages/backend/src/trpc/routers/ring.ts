...
import type { RingStatusResponse, RingAuthResponse, Ring2FAResponse, RingDevicesResponse, RingHistoryResponse } from "@smarthome/shared"

export const ringRouter = router({
  status: protectedProcedure.query(async ({ ctx }): Promise<RingStatusResponse> => {
    return {
  ...
  devices: protectedProcedure.query(async ({ ctx }): Promise<RingDevicesResponse> => {
  ...
  auth: adminProcedure
    ...
    .mutation(async ({ ctx, input }): Promise<RingAuthResponse> => {
      const result = await ringService.authenticate(ctx.user.id, input.email, input.password)
      return result
    }),
  ...
  submit2FA: adminProcedure
    ...
    .mutation(async ({ ctx, input }): Promise<Ring2FAResponse> => {
      const result = await ringService.submitTwoFactorCode(ctx.user.id, input.code)
      return result
    }),
  ...
  history: protectedProcedure
    ...
    .query(async ({ ctx, input }): Promise<RingHistoryResponse> => {
      const history = await ringService.getHistory(ctx.user.id, input.deviceId, input.limit)
      return { history }
    }),
