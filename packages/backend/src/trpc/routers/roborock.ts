...
import type { RoborockStatusResponse, RoborockDevicesResponse } from "@smarthome/shared"

export const roborockRouter = router({
  status: protectedProcedure.query(async ({ ctx }): Promise<RoborockStatusResponse> => {
    return {
  ...
  devices: protectedProcedure.query(async ({ ctx }): Promise<RoborockDevicesResponse> => {
  ...
  auth: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      await roborockService.authenticate(ctx.user.id, input.email, input.password)
      return { success: true }
    }),
  ...
  setFanSpeed: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      await roborockService.setFanSpeed(ctx.user.id, input.deviceId, input.speed)
      return { success: true }
    }),
  ...
  setWaterLevel: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      await roborockService.setWaterLevel(ctx.user.id, input.deviceId, input.level)
      return { success: true }
    }),
  ...
  cleanRooms: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      await roborockService.cleanRooms(ctx.user.id, input.deviceId, input.roomIds)
      return { success: true }
    }),
  ...
  history: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      const history = await roborockService.getCleanHistory(ctx.user.id, input.deviceId)
      return { history }
    }),
})
