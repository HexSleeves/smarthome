...
export const deviceApi = {
  ...
  events: (id: string, limit = 50) => api<{ events: any[] }>(`/devices/${id}/events?limit=${limit}`),
  recentEvents: (limit = 20) => api<{ events: any[] }>(`/devices/events/recent?limit=${limit}`),
};

// Roborock endpoints
export const roborockApi = {
  status: () => api<{ connected: boolean; hasCredentials: boolean }>('/roborock/status'),
  auth: (email: string, password: string) =>
    api('/roborock/auth', { method: 'POST', body: JSON.stringify({ email, password }) }),
  ...
  command: (deviceId: string, command: string) =>
    api(`/roborock/devices/${deviceId}/command`, { method: 'POST', body: JSON.stringify({ command }) }),
  setFanSpeed: (deviceId: string, speed: string) =>
    api(`/roborock/devices/${deviceId}/fan-speed`, { method: 'POST', body: JSON.stringify({ speed }) }),
  setWaterLevel: (deviceId: string, level: string) =>
    api(`/roborock/devices/${deviceId}/water-level`, { method: 'POST', body: JSON.stringify({ level }) }),
  cleanRooms: (deviceId: string, roomIds: number[]) =>
    api(`/roborock/devices/${deviceId}/clean-rooms`, { method: 'POST', body: JSON.stringify({ roomIds }) }),
  history: (deviceId: string) => api<{ history: any[] }>(`/roborock/devices/${deviceId}/history`),
};
...
export const ringApi = {
  ...
  snapshotUrl: (deviceId: string) => `${API_BASE}/ring/devices/${deviceId}/snapshot?token=${accessToken}`,
  history: (deviceId: string, limit = 20) => api<{ history: any[] }>(`/ring/devices/${deviceId}/history?limit=${limit}`),
  toggleLight: (deviceId: string, on: boolean) =>
    api(`/ring/devices/${deviceId}/light`, { method: 'POST', body: JSON.stringify({ on }) }),
