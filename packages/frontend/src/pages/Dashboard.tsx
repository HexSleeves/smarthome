...
export function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const unsubRoborock = wsClient.on('roborock:status', (data) => {
      setRealtimeEvents(prev => [{ type: 'vacuum_status', ...data, timestamp: new Date() }, ...prev].slice(0, 5));
    });
    
    const unsubRingMotion = wsClient.on('ring:motion', (data) => {
      setRealtimeEvents(prev => [{ type: 'motion', ...data, timestamp: new Date() }, ...prev].slice(0, 5));
    });
    
    const unsubRingDing = wsClient.on('ring:ding', (data) => {
      setRealtimeEvents(prev => [{ type: 'doorbell', ...data, timestamp: new Date() }, ...prev].slice(0, 5));
    });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4 text-sm">
        <div className={`flex items-center gap-1.5 ${wsConnected ? 'text-green-600' : 'text-gray-400'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/vacuum" className="text-sm text-primary-600 hover:text-primary-700">
              View All →
            </Link>
          </div>
        ...
        <div className="card p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/doorbell" className="text-sm text-primary-600 hover:text-primary-700">
              View All →
            </Link>
          </div>
            ...
            {realtimeEvents.map((event, i) => (
              <div key={i} className="flex items-center gap-3 p-2 rounded-lg bg-gray-50 dark:bg-gray-700/50">
                <div className={`w-2 h-2 rounded-full ${
                  event.type === 'doorbell' ? 'bg-yellow-500' :
                  event.type === 'motion' ? 'bg-blue-500' : 'bg-green-500'
                }`} />
                <span className="flex-1 text-sm">
                  {event.type === 'doorbell' ? '🔔 Doorbell pressed' :
...
function StatCard({ title, icon: Icon, value, status, link }: {
  title: string;
  icon: any;
  value: string;
...
function VacuumCard({ vacuum }: { vacuum: any }) {
  ...
  return (
    <div className="flex items-center gap-4 p-3 rounded-lg bg-gray-50 dark:bg-gray-700/50">
      <div className={`w-3 h-3 rounded-full ${statusColors[state?.status || 'offline']}`} />
      <div className="flex-1">
        <p className="font-medium">{vacuum.name}</p>
        <p className="text-sm text-gray-500 capitalize">{state?.status || 'Offline'}</p>
      </div>
...
function DoorbellCard({ doorbell }: { doorbell: any }) {
  ...
  return (
    <div className="flex items-center gap-4 p-3 rounded-lg bg-gray-50 dark:bg-gray-700/50">
      <div className={`w-3 h-3 rounded-full ${state?.status === 'online' ? 'bg-green-500' : 'bg-gray-400'}`} />
      <div className="flex-1">
        <p className="font-medium">{doorbell.name}</p>
        <p className="text-sm text-gray-500 capitalize">{state?.type || 'Doorbell'}</p>
      </div>
