...
import { Bell, Camera, Sun, AlertTriangle, Battery, Video, RefreshCw, Loader2, Clock } from 'lucide-react';
  ...
  if (!statusData?.connected && !statusData?.hasCredentials) {
    return (
      <div className="card p-8 text-center">
        ...
        <p className="text-gray-500 mb-4">Connect your Ring doorbell in Settings to get started.</p>
    ...
    <div className="space-y-6">
      {devices.length === 0 ? (
        <div className="card p-8 text-center">
          ...
          <p className="text-gray-500">Make sure your Ring device is set up in the Ring app.</p>
        </div>
      ) : (
...
function DoorbellDevice({ device, isAdmin }: { device: any; isAdmin: boolean }) {
  ...
  useEffect(() => {
    const unsubMotion = wsClient.on('ring:motion', (data) => {
      if (data.deviceId === device.id) {
        setNotifications(prev => [{ type: 'motion', time: new Date(), ...data }, ...prev].slice(0, 5));
      }
    });

    const unsubDing = wsClient.on('ring:ding', (data) => {
      if (data.deviceId === device.id) {
        setNotifications(prev => [{ type: 'ding', time: new Date(), ...data }, ...prev].slice(0, 5));
      }
    });
    ...
    <div className="card overflow-hidden">
          ...
          <div className="mt-2 space-y-1">
            {notifications.map((notif, i) => (
              <div key={i} className="text-sm text-yellow-600 dark:text-yellow-300">
                {notif.type === 'ding' ? '🔔 Doorbell pressed' : '🚶 Motion detected'}
                {' - '}
      ...
      <div className="p-6 space-y-6">
        <div>
          ...
          {!isAdmin && (
            <p className="text-sm text-gray-500 mt-2">Admin access required for controls.</p>
          )}
        </div>
                ...
                >
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-medium capitalize">
                      {event.kind === 'ding' ? 'Doorbell Press' : event.kind || 'Motion'}
                    </p>
