			...
			{status?.connected ? (
				<div className="space-y-4">
					<p className="text-gray-600 dark:text-gray-400">
						Your Roborock account is connected. Your devices should appear on
						the Dashboard and Vacuum pages.
					</p>
					<button
	...
	const authMutation = useMutation({
		...
		onSuccess: (data) => {
			if (data.requiresTwoFactor) {
				...
				setRequiresTwoFactor(true);
				setTwoFactorPrompt(
					data.prompt || "Please enter the 2FA code sent to your phone.",
				);
				setError("");
	...
	return (
		<div className="card p-6">
			...
			{status?.connected ? (
				<div className="space-y-4">
					<p className="text-gray-600 dark:text-gray-400">
						Your Ring account is connected. Your devices should appear on the
						Dashboard and Doorbell pages.
					</p>
					<button
						onClick={() => disconnectMutation.mutate()}
						disabled={disconnectMutation.isPending}
						...
						<>
							...
							<div className="flex gap-2">
								<button
									type="submit"
									disabled={
										twoFactorMutation.isPending || twoFactorCode.length < 4
									}
