...
import { Play, Pause, Square, Home, Volume2, Battery, Droplets, Wind, AlertTriangle, Loader2 } from 'lucide-react';
  ...
  if (!statusData?.connected && !statusData?.hasCredentials) {
    return (
      <div className="card p-8 text-center">
        ...
        <p className="text-gray-500 mb-4">Connect your Roborock vacuum in Settings to get started.</p>
    ...
    <div className="space-y-6">
      {devices.length === 0 ? (
        <div className="card p-8 text-center">
          ...
          <p className="text-gray-500">Make sure your vacuum is set up in the Roborock app.</p>
        </div>
      ) : (
    ...
    <div className="card overflow-hidden">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-6 bg-gray-50 dark:bg-gray-800/50">
        <div>
          ...
          <p className="text-lg font-semibold">{device.cleanArea?.toFixed(1) || 0} m²</p>
        </div>
        <div>
        ...
        <div>
          ...
          <p className="text-lg font-semibold capitalize">{device.fanSpeed || 'N/A'}</p>
        </div>
        <div>
          <p className="text-sm text-gray-500">Water Level</p>
          <p className="text-lg font-semibold capitalize">{device.waterLevel || 'N/A'}</p>
        </div>
      </div>
      ...
      <div className="p-6 space-y-6">
        ...
        <div>
          ...
          {!isAdmin && (
            <p className="text-sm text-gray-500 mt-2">Admin access required to control the vacuum.</p>
          )}
        </div>
